  ...
  <div class="touch_customers m98Auto">
    <div class="paddingTB20">
      <el-form :inline="true" :model="formInline" label-position="right" label-width="110px" size="mini" class="demo-form-inline">
        ...
        <el-form-item>
          <el-button type="primary" icon="el-icon-search" size="mini" @click="waitCustomer">查询</el-button>
        </el-form-item>
      </el-form>
    </div>
    <el-table
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <a @click="$router.push('/infoCost/infoCost')">{{ scope.row.name }}</a>
        </template>
      </el-table-column>
      <el-table-column
        prop="created_at"
...
<script>
import { waitCustomer, labelCustomer, brandCustomer, regionCustomer } from '@/api/dhkfApi'
...
export default {
  ...
  methods: {
    waitCustomer(current) {
      const that = this
      return new Promise((resolve, reject) => {
        waitCustomer(that.paramFormat(current)).then(res => {
          const { data } = res
