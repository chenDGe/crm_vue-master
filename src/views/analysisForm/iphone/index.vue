...
<!--  https://v-charts.js.org/#/-->
  <div class="m98Auto paddingTB20">
    <el-tabs v-model="elTabsId" type="border-card" @tab-click="statisticsCustomer">
      ...
      <el-tab-pane label="通话员工数" lazy>
        <ve-line :data="chartData5" :settings="chartSettings5" />
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
      ...
      return new Promise((resolve, reject) => {
        ...
        statisticsCustomer({ type: Number(that.elTabsId) + 1 }).then(res => {
          ...
          if (tabIndex === 1) {
            ...
            that.chartData5.rows = []
            for (const tmp in data) {
              const param = { '日期': tmp, '通话员工数': data[tmp] }
              that.chartData5.rows.push(param)
            }
          }
          resolve(res)
        }).catch(err => {
