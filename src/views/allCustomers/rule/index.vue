  ...
  <div class="touch_customers m98Auto">
    <div class="paddingTB20">
      <el-form :inline="true" :model="formInline" label-position="right" label-width="110px" size="mini" class="demo-form-inline">
        ...
        <el-form-item>
          <el-button type="primary" icon="el-icon-search" size="mini" @click="cateCustomer">查询</el-button>
        </el-form-item>
      </el-form>
    </div>
    <el-table
...
<script>
import { cateCustomer, detailLabelCustomer, brandCustomer, regionCustomer } from '@/api/dhkfApi'
...
export default {
  ...
  mounted() {
    this.cateCustomer()
    this.detailLabelCustomer()
    this.indexStaff()
    this.brandCustomer()
    this.regionCustomer()
  },
  methods: {
    cateCustomer(current) {
      const that = this
      return new Promise((resolve, reject) => {
        cateCustomer(that.paramFormat(current)).then(res => {
          const { data } = res
    ...
    paramFormat(current) {
      ...
      that.formInline['level'] = this.$route.meta.id
